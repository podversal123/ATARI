...
import { SelectCategoryDropdown, ALL_CATEGORIES } from "./select-category-dropdown";
import {
...
export function SuperAdminModuleImagesView() {
  ...
  const [categoryPath, setCategoryPath] = useState(ALL_CATEGORIES);
  ...
  const categoryLabel =
    categoryPath === ALL_CATEGORIES
      ? "All Categories"
      : (MODULE_IMAGE_CATEGORIES.find((c) => c.path === categoryPath)?.label ?? "All Categories");
  ...
  const filteredRows = useMemo(() => {
    ...
    return MODULE_IMAGE_ROWS.filter((row) => {
      ...
      if (categoryPath !== ALL_CATEGORIES && row.categoryPath !== categoryPath) return false;
      if (fromDate && row.date < fromDate) return false;
      if (toDate && row.date > toDate) return false;
      if (query && !(row.kvk.toLowerCase().includes(query) || row.caption.toLowerCase().includes(query))) {
        return false;
      }
      return true;
    });
  }, [reportingYear, selectedKvks, categoryPath, fromDate, toDate, search]);

  const hasActiveFilters =
    selectedKvks.size > 0 || categoryPath !== ALL_CATEGORIES || fromDate !== "" || toDate !== "" || search !== "";

  function resetFilters() {
    setSelectedKvks(new Set());
    setCategoryPath(ALL_CATEGORIES);
    setFromDate("");
    setToDate("");
    setSearch("");
  ...
  const kvkLabel = useMemo(() => {
    if (selectedKvks.size === 0 || selectedKvks.size === KVKS.length) return "All KVKs";
  ...
  function toggleAllRows() {
    setSelectedRows(allRowsSelected ? new Set() : new Set(filteredRows.map((row) => row.id)));
  }
    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-border bg-card p-4">
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground">Select KVKs</label>
            <div className="mt-1">
              <SelectKvksMultiDropdown selected={selectedKvks} onChange={setSelectedKvks} />
            </div>
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground">Category / Form</label>
            <div className="mt-1">
              <SelectCategoryDropdown value={categoryPath} onChange={setCategoryPath} />
            </div>
          </div>
        ...
        {dateRangeInvalid && (
          <p className="mt-2 text-xs text-destructive">To Date cannot be earlier than From Date.</p>
        )}
      </div>
      ...
      <div className="rounded-lg border border-border bg-card">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border p-4">
          <div>
            <p className="text-sm font-semibold text-foreground">Uploaded Images ({filteredRows.length})</p>
              ...
              <DropdownMenuContent align="end" className="w-72">
                {BULK_DOWNLOAD_OPTIONS.map((option) => (
                  <DropdownMenuItem
                    ...
                    disabled={option.mode === "selected-records" && selectedRows.size === 0}
              ...
              {filteredRows.length === 0 ? (
                ...
                filteredRows.map((row, index) => (
                  <tr key={row.id} className="divide-x divide-border border-b border-border last:border-0">
                    ...
                    <td className="px-4 py-3 align-top text-foreground">{row.kvk}</td>
                    <td className="px-4 py-3 align-top text-muted-foreground">{row.date}</td>
                    <td className="px-4 py-3 align-top text-foreground">{row.categoryLabel}</td>
                    <td className="px-4 py-3 align-top">
                      {row.previewUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={row.previewUrl} alt={row.caption} className="size-10 rounded-md object-cover" />
                      ) : (
                    ...
                    <td className="px-4 py-3 align-top text-foreground">
                      <span className="line-clamp-2 max-w-xs" title={row.caption}>
                        {row.caption}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right align-top">
                      <Button variant="ghost" size="icon-sm" disabled={!row.previewUrl}>
                        <Download className="size-3.5" />
                      </Button>
                    </td>
                  </tr>
        ...
        <div className="flex items-center justify-between border-t border-border px-4 py-3 text-sm text-muted-foreground">
          <span>
            {filteredRows.length === 0 ? "Showing 0-0 of 0" : `Showing 1-${filteredRows.length} of ${filteredRows.length}`}
          </span>
