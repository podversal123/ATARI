 ...
 */
export function SelectKvksMultiDropdown({ selected, onChange }: SelectKvksMultiDropdownProps) {
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-72 min-w-0 overflow-y-auto">
        ...
        {ALL_KVK_NAMES.map((name) => (
          <DropdownMenuCheckboxItem key={name} checked={allSelected || selected.has(name)} onCheckedChange={() => toggleOne(name)}>
            {name}
          </DropdownMenuCheckboxItem>
        ))}
      </DropdownMenuContent>
