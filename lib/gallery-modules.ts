 ...
 * screenshot of /gallery: a "MODULES" list with "ABOUT KVKS" expanded into
...
export const GALLERY_MODULES: GalleryModule[] = FORM_MANAGEMENT.map((item) => ({
  slug: item.slug,
  label: item.label,
  leaves: item.type === "group" ? collectLeaves(item.children, item.label) : [{ slug: item.slug, label: item.label, trail: item.label }],
}));
